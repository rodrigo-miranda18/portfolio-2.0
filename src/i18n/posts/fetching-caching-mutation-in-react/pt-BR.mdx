...
Como deu pra notar, a promise é passada como prop para o componente `Dashboard`. Isso se dá pois o `use` não é capaz de resolver promises criadas na renderização, portanto, elas devem ser criadas em um server component e passadas como props para client components.
