...
With just these series of automatic optimizations, the size of the image served to the user is drastically reduced. If, for example, you have an unoptimized JPG or PNG image of 700kb, you will notice that the mobile version after optimization will easily contain less than 90kb.

In addition, the Image component provides other good features:
...
In addition, if your audit tool is reporting [LCP](https://web.dev/articles/lcp) on an image, you can use the `priority` attribute to preload the image.

- **Image preload**: Unlike images that can be loaded only when the user sees them, using the Lazy Loading feature, there are images that you want to load as soon as possible, for example images of the first fold of your page. Usually these images affect the [LCP](https://web.dev/articles/lcp) of the site. Using the `priority` attribute in the Image component, Next.js adds an HTML `link` tag to preload the image, causing the browser to load the image in advance.
...
If your page is a blog post, or an e-commerce product, or any page with non-"real-time" content, and you have a large user base, there is no reason not to use static rendering.

If possible, also try to create them as much as possible at build time, and use [Incremental Static Regeneration](https://nextjs.org/docs/app/building-your-application/data-fetching/incremental-static-regeneration) to regenerate them. Creating pages at build time greatly improves the initial page load and the first users who access it.

In summary, use static rendering as much as possible.
